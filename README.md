...
There is no need to make a get request to the server in order to grab resources (pages) when navigating from one page to another.
...
#### Link Component (use this component in order to navigate from one page to another)

- the navigation from pages to pages happens on the front end. we are not making any get request to the server in order to get a resource

- in order to direct the user somewhere externally you still need to use the standard <a href='www.'>extenal link</a>

- the react router dom LINK element is just for navigating inside the page application
...
#### Nested Pages (usecase: display the same structure eg. Navbar in all pages)
...
USE OUTLET IN THE HomeLayout.jsx
...
- set up index page (remove the path and just add index:true)
...
- setting up the same style to the outlet component which contains all our pages
  HomeLayout.jsx
...
- need to add the error element to the homeLayout structure.
...
const router = createBrowserRouter([
  {
    path: "/",
    element: <HomeLayout />,
    errorElement: <Error />, // adding error element here. if the user is looking for something it soes not exist the <Error> page is going to be displayed
    children: [
      {
        index: true,
        element: <Landing />,
...
- error handling in case of wrong url.

- useRouteError is a hook, a function, to
...
- useEffect approach (use effect runs after the component mounts), however, we will use Loader which is provided by react dom library. imagine we need to fetch data with use effect. in this case the component will mount first and then the data are being fetched. on the contrary, with loader the data are pre-fetched before the component mounts

we are fetching in --> Landing.jsx
...
#### Loader (is used to replace useeffect approach)
...
- adding the id to the link as each cocktail will have a specific id.
...
const HomeLayout = () => {
  // this hook provides info about the state (whether is loading or idle). the global value can be accessed with the hook: useOutletContext()

  const navigation = useNavigation();
  const isPageLoading = navigation.state === "loading";

  const value = "some value"; //keep in mind that in latest react router dom library
  return (
    <>
      <Navbar />
      <section className='page'>
        {isPageLoading ? (
          ...
          <Outlet context={{ value }} /> //keep in mind that in latest react router dom library
        )}
      </section>
    </>
  );
...
#### Redirects if you are deploying to netlify

- in public folder create '\_redirects' file and put into the file this code
